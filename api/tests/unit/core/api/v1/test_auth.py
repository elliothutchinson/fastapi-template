...
def test_login_for_auth_token(client, mocker):
    ...
    mocker.patch(f"{UUT_PATH}.user_login", AsyncMock(return_value=auth_token))
...
def test_logout_auth_token(client, mocker):
    ...
    mocker.patch(f"{UUT_PATH}.user_logout", AsyncMock(return_value=(True, True)))
...
def test_refresh_auth_token(client, mocker):
    ...
    mocker.patch(f"{UUT_PATH}.refresh_access_token", AsyncMock(return_value=auth_token))
