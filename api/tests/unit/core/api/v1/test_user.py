...
def test_register_new_user(client, mocker):
    ...
    mocker.patch(
        "app.core.api.v1.user.user_service.create", AsyncMock(return_value=user_private)
    )
...
def test_read_current_user(client, mocker, auth_headers, override_get_user_from_token):
    ...
    mocker.patch(
        "app.core.api.v1.user.user_service.fetch", AsyncMock(return_value=user_private)
    )
...
def test_update_current_user(
    ...
    mocker.patch(
        "app.core.api.v1.user.user_service.update", AsyncMock(return_value=user_private)
    )
