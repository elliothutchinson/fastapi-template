...
@patch(
    "app.core.api.middleware.uuid4",
    Mock(return_value=UUID(request_id_str())),
)
async def test_RequestIdMiddleware_dispatch_sets_new_request_id(mocker):
    ...
    actual = uut.RequestIdMiddleware(Mock())
    actual_response = await actual.dispatch(mock_request, mock_call_next)

    assert actual_response.headers["request_id"] == request_id_str()
    mock_context_request_id.set.assert_called_once_with(request_id_str())


async def test_RequestIdMiddleware_dispatch_sets_existing_request_id(mocker):
    mock_context_request_id = Mock()
    mocker.patch("app.core.api.middleware.ctx_request_id", mock_context_request_id)
...
async def test_RequestIdMiddleware_dispatch_sets_request_ip(mocker):
    ...
    actual = uut.RequestIdMiddleware(Mock())
    await actual.dispatch(mock_request, AsyncMock())
