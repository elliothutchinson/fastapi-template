...
from app.core.api.security.service import get_authenticated_user, get_active_user
...
async def generate_login_token(user: User, with_refresh_token: bool = False) -> AccessToken:
...
async def refresh_token(token: str) -> AccessToken:
    token_data = token_service.get_verified_token_data(
        token=token, claim=REFRESH_TOKEN,
    )
    username = token_data.data["username"]

    refreshed_user = await get_active_user(username=username)
