...
async def register_new_user(user_create: UserCreate, background_tasks: BackgroundTasks):
    user = await user_service.register_user(user_create=user_create)
...
async def read_current_user(login_user: User = Depends(login_service.get_login_user)):
    return await user_service.get_user_data(username=login_user.username)
...
async def update_user(
    ...
    user = await user_service.update_user_data(
        username=login_user.username, user_update=user_update
    )
